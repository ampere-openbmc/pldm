...
class OemEventManager
{
  public:
    ...
    explicit OemEventManager(
        ...
        event(event),
        pollingTime(NORMAL_EVENT_POLLING_TIME), manager(manager){};
  ...
  protected:
     ...
     *  @param[in] presentReading - the present reading of the sensor
     */
    void handleBootOverallEvent(uint8_t /*tid*/, uint16_t /*sensorId*/,
                                uint32_t presentReading);

    /** @brief Handle numeric sensor event message from DDR or DIMM status
     * sensor.
     *
     *  @param[in] tid - TID
     *  @param[in] sensorId - Sensor ID
