...
*/
std::array<std::string, 8> pmicTempAlertMsg = { "Below 85°C", "85°C", "95°C",
                                            "105°C", "115°C", "125°C", "135°C",
                                            "Equal or greater than 140°C"};
...
std::unordered_map<uint16_t, std::vector<std::pair<std::string,
    ...
    [[maybe_unused]] size_t eventDataSize)
{
    ...
    std::string cperFile = std::string(CPER_TEMP_DIR) + "cper.dump." +
                           std::to_string(tid);
    ...
    if (ampHdr.typeId.member.isBert)
    {
        ...
        try
        {
            ...
            auto method = bus.new_method_call(rasSrv, rasPath, dbusProperties,
                                              "Set");
                                         ...
                                         log_level& logLevel)
{
    if (!description.empty())
    {
        ...
        sd_journal_send("MESSAGE=%s", description.c_str(),
                        ...
                        "REDFISH_MESSAGE_ARGS=%s", description.c_str(), NULL);
    }
}
...
void OemEventManager::handleBootOverallEvent(
    uint8_t /*tid*/, uint16_t /*sensorId*/, uint32_t presentReading)
{
    ...
    size_t sensorDataLength)
{
    ...
    {
        handleDDRStatusEvent(tid, sensorId, presentReading, true);
        return PLDM_SUCCESS;
    }
    ...
    switch (sensorId)
    {
        ...
        case PCIE_HOT_PLUG:
            ...
            break;
        case DDR_STATUS:
            handleDDRStatusEvent(tid, sensorId, presentReading, false);
            break;
        case PCP_VR_STATE:
        case SOC_VR_STATE:
        case DPHY_VR1_STATE:
        ...
        else
        {
            switch (byte3)
            {
                ...
                case dimm_status::TSx_HIGH_TEMP:
                    ...
                    description +=
                        " exceeding their high temperature threshold";
                    break;
...
void OemEventManager::handleNumericWatchdogEvent(pldm_tid_t tid, uint16_t sensorId,
                                                uint32_t presentReading)
{
    std::string description;
