...
void SensorManager::startPolling(pldm_tid_t tid)
{
    ...
    {
        lg2::info("Terminus ID {TID}: Failed to start sensor polling timer.",
                  "TID", tid);
        return;
    }
}
...
void SensorManager::stopPolling(pldm_tid_t tid)
{
    ...
    if (forceStopSensorPoll.contains(tid))
    {
        forceStopSensorPoll.erase(tid);
    }
}
...
void SensorManager::doSensorPolling(pldm_tid_t tid)
{
    ...
    scope.spawn(stdexec::just() |
        ...
        else
        {
            ...
            {
                lg2::info("Terminus ID {TID}: Failed to stop polling timer.",
                          "TID", tid);
            }
            rcOpt = PLDM_SUCCESS;
        }
...
exec::task<int> SensorManager::doSensorPollingTask(pldm_tid_t tid)
{
    ...
    do
    {
        ...
        /* Stop sensor polling but still allow event polling */
        if ((!forceStopSensorPoll.contains(tid)) || forceStopSensorPoll[tid])
        {
            lg2::info("Terminus ID {TID}: Force stop sensor polling at {NOW}.",
                      "TID", tid, "NOW", pldm::utils::getCurrentSystemTime());
        ...
        {
            if ((!forceStopSensorPoll.contains(tid)) ||
                forceStopSensorPoll[tid])
            {
                lg2::info(
                    "Terminus ID {TID}: Force stop sensor polling at {NOW}.",
                    "TID", tid, "NOW", pldm::utils::getCurrentSystemTime());
                co_await stdexec::just_stopped();
            }
