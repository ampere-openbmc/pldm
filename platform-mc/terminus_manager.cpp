...
{
    if (std::get<0>(mctpInfo) == MCTP_NULL_EID ||
        std::get<0>(mctpInfo) < MCTP_START_VALID_EID ||
        std::get<0>(mctpInfo) == MCTP_BROADCAST_EID)
    {
        return std::nullopt;
    }
...
bool TerminusManager::unmapTid(const pldm_tid_t& tid)
{
    ...
    auto transportLayerTableIt = transportLayerTable.find(tid);
    if (transportLayerTableIt != transportLayerTable.end())
    {
        transportLayerTable.erase(transportLayerTableIt);
    }

    auto mctpInfoIt = mctpInfoTable.find(tid);
    if (mctpInfoIt != mctpInfoTable.end())
    {
        mctpInfoTable.erase(mctpInfoIt);
    }
...
exec::task<int> TerminusManager::setDateTime(pldm_tid_t tid)
{
    ...
    lg2::info("SetDateTime {DATE}/{MONTH} {HOUR}:{MIN}:{SEC}", "DATE", lg2::hex,
              day, "MONTH", lg2::hex, month, "HOUR", lg2::hex, hours, "MIN",
              lg2::hex, minutes, "SEC", lg2::hex, seconds);

    /* instanceId will be include in sendRecvPldmMsg */
    auto rc = encode_set_date_time_req(0, seconds, minutes, hours, day, month,
                                       year, requestMsg,
                                       sizeof(struct pldm_set_date_time_req));
    if (rc)
    {
