...
class SensorManager
{
  public:
    ...
    /** @brief set force stop flag to stop sensor polling task when it is in
     *         the middle of sensor reading flow.
     */
    void setForceStop(pldm_tid_t tid)
    {
        if (forceStopSensorPoll.contains(tid))
        {
            forceStopSensorPoll[tid] = true;
        }
    };
  ...
  protected:
    ...
    std::map<pldm_tid_t, bool> forceStopSensorPoll;
