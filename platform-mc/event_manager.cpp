                                                       ...
                                                       uint16_t pollEventId)
{
    ...
    while (eventId != 0)
    {
        ...
        /* Stop event polling */
        if ((!forceStopEventPoll.contains(tid)) || forceStopEventPoll[tid])
        {
            lg2::info("Terminus ID {TID}: Force stop event polling at {NOW}.",
